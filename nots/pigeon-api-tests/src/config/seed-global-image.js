...
// name (e.g. <img data-image-name="owl">). Names are stable (no timestamp) so the seed is
// idempotent — a re-run reuses the already-seeded image instead of failing on a duplicate.
...
async function seedImage({ name, file }) {
  step(`Seed global image "${name}" (${file})`);
  try {
    ...
    ok(`Created "${name}": ${image.id}`);
    return image.id;
  } catch (e) {
    // A previous run may have already seeded it; reuse the existing image's id so the
    // seed stays idempotent.
    const existing = await pigeon.queryGlobalImages({ name, accessToken: GLOBAL_TOKEN });
...
async function run() {
  ...
  for (const image of IMAGES) {
    const id = await seedImage(image);
    if (id) {
      seededIds.push(id);
    }
  }

  console.log(`\nSeeded global images ${seededIds.join(", ")} (left in database).\n`);

  if (!summary("global images seed")) {
    process.exitCode = 1;
  }
}
